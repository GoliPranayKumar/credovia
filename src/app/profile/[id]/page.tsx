...
export default function PublicProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchData = async () => {
    ...
    try {
      const pRes = await withRetry(() => databases.getDocument(DATABASE_ID, USERS_COLLECTION_ID, id))
        .catch(async (err) => {
      ...
      let p = pRes as any;
      const { total: newScore, enrichedReviews } = await recalculateAndSyncScore(id, p, true);
      ...
      setProfile(p);
      setCachedData(cacheKey, { profile: p, reviews: enrichedReviews || [] });
    } catch (err) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="p-8 rounded-[2.5rem] bg-white border border-border/60 shadow-sm space-y-6">
            ...
            <ScoreBreakdownView breakdown={calculateCredibilityScore(profile as any).breakdown} currentScore={profile.score} />
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {r.reviewerScore > 0 && (
                            <span className="px-1.5 py-0.5 rounded text-[9px] font-black bg-violet-100 text-violet-700">
                              Score: {r.reviewerScore}
                            </span>
                          )}
                        </div>
