...
export default function PublicProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <div className="min-h-[60vh] flex items-center justify-center"><Loader2 className="w-12 h-12 animate-spin text-violet-500" /></div>;
  ...
  return (
    <div className="space-y-12 max-w-5xl mx-auto">
      ...
      <section className="p-8 md:p-12 rounded-[3rem] bg-white border border-border/60 shadow-sm flex flex-col md:flex-row gap-10 items-center md:items-start">
        <div className="w-40 h-40 rounded-[2.5rem] bg-gradient-to-br from-violet-500 to-cyan-500 flex items-center justify-center text-6xl font-black text-white shadow-2xl shadow-violet-500/20">
          {profile.name[0]}
        </div>
        ...
        <div className="flex-1 text-center md:text-left space-y-4">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-4">
             {profile.github && <a href={profile.github} target="_blank" className="flex items-center gap-2 bg-violet-50 px-4 py-2 rounded-xl border border-violet-200 hover:bg-violet-100 transition-colors text-violet-800"><Github className="w-5 h-5"/> GitHub</a>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
        ...
        <div className="lg:col-span-2 space-y-8">
           <div className="p-8 rounded-[2.5rem] bg-white border border-border/60 shadow-sm space-y-6">
                ...
                <div className="space-y-4 pt-4">
                    ...
                    {profile.walletAddress && (
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-cyan-600 font-bold">ALPHA VERIFIED</span>
                      </div>
                    )}
                    <div className="flex justify-between text-sm">
                ...
                <div className="p-4 rounded-2xl bg-violet-50 border border-violet-100">
                    <p className="text-[11px] text-violet-700 font-medium italic leading-relaxed">
           ...
           {user && user.$id !== id && (
              <motion.div 
                ...
                className="p-8 rounded-[2.5rem] bg-violet-50 border border-violet-200 space-y-6"
              >
                ...
                <form onSubmit={submitReview} className="space-y-4">
                  ...
                  <textarea 
                    required
                    className="w-full bg-white border border-violet-200 rounded-2xl p-4 min-h-[100px] outline-none focus:ring-2 focus:ring-violet-400 transition-all text-sm"
                  ...
                  <button 
                     type="submit" 
                     disabled={submitting}
                     className="w-full bg-gradient-to-r from-violet-600 to-cyan-500 text-white py-3 rounded-2xl font-bold shadow-lg shadow-violet-500/15 flex items-center justify-center gap-2 hover:from-violet-700 hover:to-cyan-600 transition-all"
                  >
        ...
        <div className="lg:col-span-3 space-y-8">
           <div className="flex items-center justify-between">
              ...
              <span className="bg-violet-50 px-4 py-1 rounded-full text-sm font-bold border border-violet-200 text-violet-700">{reviews.length} Reviews</span>
           </div>
             ...
             {reviews.length > 0 ? (
                reviews.map((r, i) => (
                  <motion.div 
                    ...
                    className="p-6 rounded-[2rem] bg-white border border-border/60 hover:border-violet-200 hover:shadow-sm transition-all space-y-3"
                  >
                    <div className="flex justify-between items-start">
                       <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-violet-400 to-cyan-400 flex items-center justify-center font-bold text-sm text-white">
                             {(r.reviewerName || "U")[0]}
                          </div>
                          <div>
                ...
                <div className="text-center py-12 bg-white rounded-[2.5rem] border-dashed border-2 border-violet-200 opacity-50">
