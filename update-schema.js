...
const COLLECTION_ID = "69b3b5b112d780c264c5"; // Grabbed from .env.local in previous step
...
async function updateSchema() {
    const stringAttributes = [
        { key: "githubCreatedAt", type: "string", size: 64, required: false },
    ];

    const intAttributes = [
        { key: "githubRepoCount", type: "integer", required: false, default: 0 },
        { key: "githubStarCount", type: "integer", required: false, default: 0 },
        { key: "githubFollowerCount", type: "integer", required: false, default: 0 },
    ];

    for (const attr of stringAttributes) {
        console.log(`Adding string attribute: ${attr.key}`);
        await fetch(`${ENDPOINT}/databases/${DATABASE_ID}/collections/${COLLECTION_ID}/attributes/string`, {
