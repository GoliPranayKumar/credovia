const PROJECT_ID = "69b3b08f003c9211b0b8";
const API_KEY = "standard_";
const ENDPOINT = "https://sgp.cloud.appwrite.io/v1";
const DATABASE_ID = "69b3b1ae003ab31e28fa";
const COLLECTION_ID = "69b3b5b112d780c264c5"; 

const HEADERS = {
    "Content-Type": "application/json",
    "X-Appwrite-Project": PROJECT_ID,
    "X-Appwrite-Key": API_KEY
};

async function updateSchema() {
    const intAttributes = [
        { key: "githubPRCount", type: "integer", required: false, default: 0 },
    ];

    for (const attr of intAttributes) {
        console.log(`Adding integer attribute: ${attr.key}`);
        const response = await fetch(`${ENDPOINT}/databases/${DATABASE_ID}/collections/${COLLECTION_ID}/attributes/integer`, {
            method: "POST", headers: HEADERS, body: JSON.stringify(attr)
        });
        const result = await response.json();
        console.log("Result:", result.message || JSON.stringify(result));
        await new Promise(r => setTimeout(r, 2000));
    }

    console.log("Schema update 5 (githubPRCount) finished!");
}

updateSchema();
