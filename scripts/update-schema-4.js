...
const COLLECTION_ID = "69b3b5b112d780c264c5"; // Grabbed from setup logs
...
async function updateSchema() {
    const boolAttributes = [
        { key: "domainVerified", required: false, default: false },
    ];

    for (const attr of boolAttributes) {
        console.log(`Adding boolean attribute: ${attr.key}`);
        const response = await fetch(`${ENDPOINT}/databases/${DATABASE_ID}/collections/${COLLECTION_ID}/attributes/boolean`, {
    ...
    console.log("Schema update 4 (domainVerified) finished!");
}
